...
// https://github.com/QuEST-Kit/QuEST/blob/master/LICENCE.txt for details

/** @file
 * An implementation of the backend in ../QuEST_ops.h for an MPI environment.
 * Mostly pure-state wrappers for the local/distributed functions implemented in
 * QuEST_cpu
 *
 * @author Ania Brown
 * @author Tyson Jones
...
Complex statevec_calcInnerProduct(Qureg bra, Qureg ket) {
  ...
  if (bra.numChunks == 1) return localInnerProd;

  qreal localReal = localInnerProd.real;
  qreal localImag = localInnerProd.imag;
  qreal globalReal, globalImag;
  MPI_Allreduce(&localReal, &globalReal, 1, MPI_QuEST_REAL, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(&localImag, &globalImag, 1, MPI_QuEST_REAL, MPI_SUM,
                MPI_COMM_WORLD);
...
qreal densmatr_calcTotalProb(Qureg qureg) {
  // computes the trace by summing every element ("diag") with global index (2^n
  // + 1)i for i in [0, 2^n-1]

  // computes first local index containing a diagonal element
  long long int diagSpacing = 1LL + (1LL << qureg.numQubitsRepresented);
  long long int numPrevDiags =
      (qureg.chunkId > 0)
          ? 1 + (qureg.chunkId * qureg.numAmpsPerChunk) / diagSpacing
          : 0;
  ...
  for (index = localIndNextDiag; index < qureg.numAmpsPerChunk;
       index += diagSpacing) {
  ...
  if (qureg.numChunks > 1)
    MPI_Allreduce(&rankTotal, &globalTotal, 1, MPI_QuEST_REAL, MPI_SUM,
                  MPI_COMM_WORLD);
  else
...
qreal statevec_calcTotalProb(Qureg qureg) {
  ...
  for (index = 0; index < numAmpsPerRank; index++) {
    // Perform pTotal+=qureg.stateVec.real[index]*qureg.stateVec.real[index]; by
    // Kahan
    ...
    c = (t - pTotal) - y;
    pTotal = t;
    // Perform pTotal+=qureg.stateVec.imag[index]*qureg.stateVec.imag[index]; by
    // Kahan
...
static int isChunkToSkipInFindPZero(int chunkId, long long int chunkSize,
                                    int measureQubit);
static int chunkIsUpper(int chunkId, long long int chunkSize, int targetQubit);
static int chunkIsUpperInOuterBlock(int chunkId, long long int chunkSize,
                                    int targetQubit, int numQubits);
static void getRotAngle(int chunkIsUpper, Complex *rot1, Complex *rot2,
                        Complex alpha, Complex beta);
static int getChunkPairId(int chunkIsUpper, int chunkId,
                          long long int chunkSize, int targetQubit);
static int getChunkOuterBlockPairId(int chunkIsUpper, int chunkId,
                                    long long int chunkSize, int targetQubit,
                                    int numQubits);
...
void syncQuESTEnv(QuESTEnv env) { MPI_Barrier(MPI_COMM_WORLD); }
 ...
 */
static void getRotAngleFromUnitaryMatrix(int chunkIsUpper, Complex *rot1,
                                         Complex *rot2, ComplexMatrix2 u) {
  if (chunkIsUpper) {
...
static int getChunkOuterBlockPairIdForPart3(
    int chunkIsUpperSmallerQubit, int chunkIsUpperBiggerQubit, int chunkId,
    long long int chunkSize, int smallerQubit, int biggerQubit, int numQubits) {
  long long int sizeOuterHalfBlockBiggerQubit = 1LL
                                                << (biggerQubit + numQubits);
  long long int sizeOuterHalfBlockSmallerQubit = 1LL
                                                 << (smallerQubit + numQubits);
  int chunksPerOuterHalfBlockSmallerQubit =
      sizeOuterHalfBlockSmallerQubit / chunkSize;
  int chunksPerOuterHalfBlockBiggerQubit =
      sizeOuterHalfBlockBiggerQubit / chunkSize;
  int rank;
...
static int densityMatrixBlockFitsInChunk(long long int chunkSize, int numQubits,
                                         int targetQubit) {
  long long int sizeOuterHalfBlock = 1LL << (targetQubit + numQubits);
...
void copyVecIntoMatrixPairState(Qureg matr, Qureg vec) {
  ...
  // and each node stores (possibly) multiple columns (vec.numAmpsPerChunk as
  // many), `vec` can be fit entirely inside a single node's matr.pairStateVec
  // (with excess!)

  // copy this node's vec segment into this node's matr pairState (in the right
  // spot)
  long long int numLocalAmps = vec.numAmpsPerChunk;
  long long int myOffset = vec.chunkId * numLocalAmps;
  memcpy(&matr.pairStateVec.real[myOffset], vec.stateVec.real,
         numLocalAmps * sizeof(qreal));
  memcpy(&matr.pairStateVec.imag[myOffset], vec.stateVec.imag,
         numLocalAmps * sizeof(qreal));
  ...
  if (numLocalAmps < maxMsgSize) maxMsgSize = numLocalAmps;
  ...
  for (int broadcaster = 0; broadcaster < vec.numChunks; broadcaster++) {
    ...
    for (int i = 0; i < numMsgs; i++) {
      ...
      MPI_Bcast(&matr.pairStateVec.real[otherOffset + i * maxMsgSize],
                maxMsgSize, MPI_QuEST_REAL, broadcaster, MPI_COMM_WORLD);
      MPI_Bcast(&matr.pairStateVec.imag[otherOffset + i * maxMsgSize],
                maxMsgSize, MPI_QuEST_REAL, broadcaster, MPI_COMM_WORLD);
    }
  }
}
...
qreal densmatr_calcFidelity(Qureg qureg, Qureg pureState) {
  ...
  // sum each localSum
  qreal globalSum;
  MPI_Allreduce(&localSum, &globalSum, 1, MPI_QuEST_REAL, MPI_SUM,
                MPI_COMM_WORLD);
...
void exchangeStateVectors(Qureg qureg, int pairRank) {
  ...
  for (i = 0; i < numMessages; i++) {
    ...
    MPI_Sendrecv(&qureg.stateVec.real[offset], maxMessageCount, MPI_QuEST_REAL,
                 pairRank, TAG, &qureg.pairStateVec.real[offset],
                 maxMessageCount, MPI_QuEST_REAL, pairRank, TAG, MPI_COMM_WORLD,
                 &status);
    // printf("rank: %d err: %d\n", qureg.rank, err);
    MPI_Sendrecv(&qureg.stateVec.imag[offset], maxMessageCount, MPI_QuEST_REAL,
                 pairRank, TAG, &qureg.pairStateVec.imag[offset],
                 maxMessageCount, MPI_QuEST_REAL, pairRank, TAG, MPI_COMM_WORLD,
                 &status);
  }
}
...
void exchangePairStateVectorHalves(Qureg qureg, int pairRank) {
  ...
  // this for all cases to be safe
  long long int maxMessageCount = MPI_MAX_AMPS_IN_MSG;
  if (numAmpsToSend < maxMessageCount) maxMessageCount = numAmpsToSend;

  // safely assume MPI_MAX... = 2^n, so division always exact
  int numMessages = numAmpsToSend / maxMessageCount;
  int i;
  long long int offset;
  // send the bottom half of my state vector to the top half of pairRank's
  // qureg.pairStateVec receive pairRank's state vector into the top of
  // qureg.pairStateVec
  for (i = 0; i < numMessages; i++) {
    offset = i * maxMessageCount;
    MPI_Sendrecv(&qureg.pairStateVec.real[offset + numAmpsToSend],
                 maxMessageCount, MPI_QuEST_REAL, pairRank, TAG,
                 &qureg.pairStateVec.real[offset], maxMessageCount,
                 MPI_QuEST_REAL, pairRank, TAG, MPI_COMM_WORLD, &status);
    // printf("rank: %d err: %d\n", qureg.rank, err);
    MPI_Sendrecv(&qureg.pairStateVec.imag[offset + numAmpsToSend],
                 maxMessageCount, MPI_QuEST_REAL, pairRank, TAG,
                 &qureg.pairStateVec.imag[offset], maxMessageCount,
                 MPI_QuEST_REAL, pairRank, TAG, MPI_COMM_WORLD, &status);
  }
}

// TODO -- decide where this function should go. It is a preparation for MPI
...
void compressPairVectorForSingleQubitDepolarise(Qureg qureg,
  ...
  long long int thisInnerBlock,  // current block
      ...
      thisIndex,  // current index in (density matrix representation) state
                  // vector
      thisIndexInOuterColumn, thisIndexInInnerBlock;
...
#pragma omp parallel shared(                                                  \
    sizeInnerBlock, sizeInnerHalfBlock, sizeOuterColumn, sizeOuterHalfColumn, \
    qureg) private(thisTask, thisInnerBlock, thisOuterColumn, thisIndex,      \
                   thisIndexInOuterColumn, thisIndexInInnerBlock, outerBit)
#endif
  {
#ifdef _OPENMP
#pragma omp for schedule(static)
#endif
    // thisTask iterates over half the elements in this process' chunk of the
    // density matrix treat this as iterating over all columns, then iterating
    // over half the values within one column. If this function has been called,
    // this process' chunk contains half an outer block or less
    for (thisTask = 0; thisTask < numTasks; thisTask++) {
      // we want to process all columns in the density matrix,
      // updating the values for half of each column (one half of each inner
      // block)
      thisOuterColumn = thisTask / sizeOuterHalfColumn;
      thisIndexInOuterColumn =
          thisTask &
          (sizeOuterHalfColumn - 1);  // thisTask % sizeOuterHalfColumn
      ...
      thisIndexInInnerBlock =
          thisTask & (sizeInnerHalfBlock - 1);  // thisTask % sizeInnerHalfBlock
      thisIndex = thisOuterColumn * sizeOuterColumn +
                  thisInnerBlock * sizeInnerBlock + thisIndexInInnerBlock;
      // check if we are in the upper or lower half of an outer block
      outerBit = extractBit(
          targetQubit, (thisIndex + qureg.numAmpsPerChunk * qureg.chunkId) >>
                           qureg.numQubitsRepresented);
      // if we are in the lower half of an outer block, shift to be in the lower
      // half of the inner block as well (we want to dephase |0><0| and |1><1|
      // only)
      thisIndex += outerBit * (sizeInnerHalfBlock);

      // NOTE: at this point thisIndex should be the index of the element we
      // want to dephase in the chunk of the state vector on this process, in
      // the density matrix representation. thisTask is the index of the pair
      // element in pairStateVec we will populate the second half of
      // pairStateVec with this process' data to send

      qureg.pairStateVec.real[thisTask + numTasks] =
          qureg.stateVec.real[thisIndex];
      qureg.pairStateVec.imag[thisTask + numTasks] =
          qureg.stateVec.imag[thisIndex];
    }
  }
}
...
void compressPairVectorForTwoQubitDepolarise(Qureg qureg, const int targetQubit,
  ...
  long long int thisInnerBlockQ2,
      ...
      thisIndex,  // current index in (density matrix representation) state
                  // vector
      thisIndexInOuterColumn, thisIndexInInnerBlockQ1, thisIndexInInnerBlockQ2,
...
#ifdef _OPENMP
#pragma omp parallel shared(                                                \
    sizeInnerBlockQ1, sizeInnerHalfBlockQ1, sizeInnerQuarterBlockQ2,        \
    sizeInnerHalfBlockQ2, sizeInnerBlockQ2, sizeOuterColumn,                \
    sizeOuterQuarterColumn,                                                 \
    qureg) private(thisTask, thisInnerBlockQ2, thisOuterColumn, thisIndex,  \
                   thisIndexInOuterColumn, thisIndexInInnerBlockQ1,         \
                   thisIndexInInnerBlockQ2, thisInnerBlockQ1InInnerBlockQ2, \
                   outerBitQ1, outerBitQ2)
#endif
  {
#ifdef _OPENMP
#pragma omp for schedule(static)
    ...
    for (thisTask = 0; thisTask < numTasks; thisTask++) {
      ...
      outerBitQ2 = extractBit(
          qubit2, (thisIndex + qureg.numAmpsPerChunk * qureg.chunkId) >>
                      qureg.numQubitsRepresented);
      // if we are in the lower half of an outer block, shift to be in the lower
      // half of the inner block as well (we want to dephase |0><0| and |1><1|
      // only)
      ...
      // want to dephase in the chunk of the state vector on this process, in
      // the density matrix representation. thisTask is the index of the pair
      // element in pairStateVec

      // state[thisIndex] = (1-depolLevel)*state[thisIndex] +
      // depolLevel*(state[thisIndex]
      //      + pair[thisTask])/2
      qureg.pairStateVec.real[thisTask + numTasks * 2] =
          qureg.stateVec.real[thisIndex];
      qureg.pairStateVec.imag[thisTask + numTasks * 2] =
          qureg.stateVec.imag[thisIndex];
    }
  }
}

void densmatr_mixDepolarising(Qureg qureg, const int targetQubit,
                              qreal depolLevel) {
  if (depolLevel == 0) return;

  int rankIsUpper;  // rank is in the upper half of an outer block
  int pairRank;     // rank of corresponding chunk

  int useLocalDataOnly = densityMatrixBlockFitsInChunk(
      qureg.numAmpsPerChunk, qureg.numQubitsRepresented, targetQubit);
  ...
  if (useLocalDataOnly) {
    ...
    rankIsUpper =
        chunkIsUpperInOuterBlock(qureg.chunkId, qureg.numAmpsPerChunk,
                                 targetQubit, qureg.numQubitsRepresented);
    pairRank = getChunkOuterBlockPairId(rankIsUpper, qureg.chunkId,
                                        qureg.numAmpsPerChunk, targetQubit,
                                        qureg.numQubitsRepresented);
...
void densmatr_mixTwoQubitDepolarising(Qureg qureg, int qubit1, int qubit2,
                                      qreal depolLevel) {
  if (depolLevel == 0) return;
  ...
  if (fabs(4*delta*(1+delta)*gamma-depolLevel)>1e-5){
      printf("Numerical error in delta; for small error rates try Taylor
  expansion.\n"); exit(1);
  }
  */
  ...
  useLocalDataOnlyBigQubit = densityMatrixBlockFitsInChunk(
      qureg.numAmpsPerChunk, qureg.numQubitsRepresented, biggerQubit);
  if (useLocalDataOnlyBigQubit) {
    ...
    useLocalDataOnlySmallQubit = densityMatrixBlockFitsInChunk(
        qureg.numAmpsPerChunk, qureg.numQubitsRepresented, smallerQubit);
    if (useLocalDataOnlySmallQubit) {
      // do part 1 locally
      densmatr_mixTwoQubitDepolarisingLocalPart1(qureg, smallerQubit,
                                                 biggerQubit, delta);

      // do parts 2 and 3 distributed (if part 2 is distributed part 3 is also
      // distributed) part 2 will be distributed and the value of the small
      // qubit won't matter
      compressPairVectorForTwoQubitDepolarise(qureg, smallerQubit, biggerQubit);
      rankIsUpperBiggerQubit =
          chunkIsUpperInOuterBlock(qureg.chunkId, qureg.numAmpsPerChunk,
                                   biggerQubit, qureg.numQubitsRepresented);
      pairRank = getChunkOuterBlockPairId(rankIsUpperBiggerQubit, qureg.chunkId,
                                          qureg.numAmpsPerChunk, biggerQubit,
                                          qureg.numQubitsRepresented);

      exchangePairStateVectorHalves(qureg, pairRank);
      densmatr_mixTwoQubitDepolarisingDistributed(
          qureg, smallerQubit, biggerQubit, delta, GAMMA_PARTS_1_OR_2);

      // part 3 will be distributed but involve rearranging for the smaller
      // qubit
      compressPairVectorForTwoQubitDepolarise(qureg, smallerQubit, biggerQubit);
      rankIsUpperBiggerQubit =
          chunkIsUpperInOuterBlock(qureg.chunkId, qureg.numAmpsPerChunk,
      ...
      densmatr_mixTwoQubitDepolarisingQ1LocalQ2DistributedPart3(
          qureg, smallerQubit, biggerQubit, delta, gamma);
    } else {
      // do part 1, 2 and 3 distributed
      // part 1
      compressPairVectorForTwoQubitDepolarise(qureg, smallerQubit, biggerQubit);
      rankIsUpperSmallerQubit =
          chunkIsUpperInOuterBlock(qureg.chunkId, qureg.numAmpsPerChunk,
                                   smallerQubit, qureg.numQubitsRepresented);
      pairRank = getChunkOuterBlockPairId(
          rankIsUpperSmallerQubit, qureg.chunkId, qureg.numAmpsPerChunk,
          smallerQubit, qureg.numQubitsRepresented);
      ...
      // part 3
      compressPairVectorForTwoQubitDepolarise(qureg, smallerQubit, biggerQubit);
      pairRank = getChunkOuterBlockPairIdForPart3(
          rankIsUpperSmallerQubit, rankIsUpperBiggerQubit, qureg.chunkId,
          qureg.numAmpsPerChunk, smallerQubit, biggerQubit,
          qureg.numQubitsRepresented);
      exchangePairStateVectorHalves(qureg, pairRank);
      densmatr_mixTwoQubitDepolarisingDistributed(qureg, smallerQubit,
                                                  biggerQubit, delta, gamma);
    }
  }
}
...
void statevec_controlledCompactUnitary(Qureg qureg, const int controlQubit,
  ...
  if (useLocalDataOnly) {
    // all values required to update state vector lie in this rank
    statevec_controlledCompactUnitaryLocal(qureg, controlQubit, targetQubit,
                                           alpha, beta);
  } else {
    // need to get corresponding chunk of state vector from other rank
    rankIsUpper =
    ...
    if (rankIsUpper) {
      statevec_controlledCompactUnitaryDistributed(qureg, controlQubit, rot1,
                                                   rot2,
                                                   qureg.stateVec,      // upper
                                                   qureg.pairStateVec,  // lower
                                                   qureg.stateVec);  // output
...
void statevec_multiControlledUnitary(Qureg qureg, long long int ctrlQubitsMask,
  ...
  if (useLocalDataOnly) {
    // all values required to update state vector lie in this rank
    statevec_multiControlledUnitaryLocal(qureg, targetQubit, ctrlQubitsMask,
                                         ctrlFlipMask, u);
  } else {
    // need to get corresponding chunk of state vector from other rank
    rankIsUpper =
    ...
    if (rankIsUpper) {
      statevec_multiControlledUnitaryDistributed(
          qureg, targetQubit, ctrlQubitsMask, ctrlFlipMask, rot1, rot2,
          qureg.stateVec,      // upper
          qureg.pairStateVec,  // lower
          qureg.stateVec);     // output
    } else {
...
void statevec_pauliX(Qureg qureg, const int targetQubit) {
  ...
  if (useLocalDataOnly) {
    ...
    exchangeStateVectors(qureg, pairRank);
    // this rank's values are either in the upper of lower half of the block.
    // pauliX just replaces this rank's values with pair values
...
qreal statevec_calcProbOfOutcome(Qureg qureg, const int measureQubit,
  ...
  int skipValuesWithinRank =
      halfMatrixBlockFitsInChunk(qureg.numAmpsPerChunk, measureQubit);
  if (skipValuesWithinRank) {
    stateProb = statevec_findProbabilityOfZeroLocal(qureg, measureQubit);
  } else {
    if (!isChunkToSkipInFindPZero(qureg.chunkId, qureg.numAmpsPerChunk,
                                  measureQubit)) {
...
qreal densmatr_calcProbOfOutcome(Qureg qureg, const int measureQubit,
  ...
  qreal outcomeProb;
  MPI_Allreduce(&zeroProb, &outcomeProb, 1, MPI_QuEST_REAL, MPI_SUM,
                MPI_COMM_WORLD);
  if (outcome == 1) outcomeProb = 1.0 - outcomeProb;
...
qreal densmatr_calcPurity(Qureg qureg) {
  ...
  qreal globalPurity;
  MPI_Allreduce(&localPurity, &globalPurity, 1, MPI_QuEST_REAL, MPI_SUM,
                MPI_COMM_WORLD);
...
void statevec_collapseToKnownProbOutcome(Qureg qureg, const int measureQubit,
  ...
  int skipValuesWithinRank =
      halfMatrixBlockFitsInChunk(qureg.numAmpsPerChunk, measureQubit);
  if (skipValuesWithinRank) {
    statevec_collapseToKnownProbOutcomeLocal(qureg, measureQubit, outcome,
                                             totalStateProb);
  } else {
    if (!isChunkToSkipInFindPZero(qureg.chunkId, qureg.numAmpsPerChunk,
                                  measureQubit)) {
      ...
      if (outcome == 0)
        statevec_collapseToKnownProbOutcomeDistributedRenorm(
            qureg, measureQubit, totalStateProb);
      else
...
long long int getGlobalIndOfOddParityInChunk(Qureg qureg, int qb1, int qb2) {
  ...
  long long int chunkEndInd =
      chunkStartInd + qureg.numAmpsPerChunk;  // exclusive
...
void statevec_swapQubitAmps(Qureg qureg, int qb1, int qb2) {
  ...
  long long int oddParityGlobalInd =
      getGlobalIndOfOddParityInChunk(qureg, qb1, qb2);
  if (oddParityGlobalInd == -1) return;

  // determine and swap amps with pair node
  int pairRank =
      flipBit(flipBit(oddParityGlobalInd, qb1), qb2) / qureg.numAmpsPerChunk;
  exchangeStateVectors(qureg, pairRank);
  statevec_swapQubitAmpsDistributed(qureg, pairRank, qb1, qb2);
}

/** This calls swapQubitAmps only when it would involve a distributed
 * communication; if the qubit chunks already fit in the node, it operates the
 * unitary direct. Note the order of q1 and q2 in the call to
 * twoQubitUnitaryLocal is important.
 *
 * @TODO: refactor so that the 'swap back' isn't performed; instead the qubit
 * locations are updated.
 * @TODO: the double swap (q1,q2 to 0,1) may be possible simultaneously by a
 * bespoke swap routine.
 */
void statevec_multiControlledTwoQubitUnitary(Qureg qureg,
  ...
  if (q1FitsInNode && q2FitsInNode) {
    ...
    statevec_multiControlledTwoQubitUnitaryLocal(qureg, ctrlMask, swap1, swap2,
                                                 u);
    statevec_swapQubitAmps(qureg, q1, swap1);
    statevec_swapQubitAmps(qureg, q2, swap2);
  }
...
/** This calls swapQubitAmps only when it would involve a distributed
 * communication; if the qubit chunks already fit in the node, it operates the
 * unitary direct. It is already gauranteed here that all target qubits can fit
 * on each node (this is validated in the front-end)
 *
 * @TODO: refactor so that the 'swap back' isn't performed; instead the qubit
 * locations are updated.
 */
void statevec_multiControlledMultiQubitUnitary(Qureg qureg,
  ...
  int freeQb = 0;
  while (maskContainsBit(targMask, freeQb)) freeQb++;
  ...
  for (int t = 0; t < numTargs; t++) {
    if (halfMatrixBlockFitsInChunk(qureg.numAmpsPerChunk, targs[t]))
      ...
      if (maskContainsBit(ctrlMask, swapTargs[t]))
        ctrlMask = flipBit(flipBit(ctrlMask, swapTargs[t]),
                           targs[t]);  // swap targ and ctrl
  ...
  statevec_multiControlledMultiQubitUnitaryLocal(qureg, ctrlMask, swapTargs,
                                                 numTargs, u);
